...
public class BaseTest {
	...
	@BeforeSuite
	public synchronized void reports() {
	...
	@Parameters({ "browser" })
	@BeforeMethod
	public synchronized void setup(@Optional("chrome") String browserName, Method method) throws InterruptedException {
	...
	public synchronized void testReportNew(ITestResult result, Hashtable<String, Object> data) {
		if (result.getStatus() == 1) {
	...
	public static void skipTest(String validationMessage) {
	...
	public void tearDown(ITestResult result) {
		...
		((SeleniumWrapper) seleniumWrapperManager.get()).wait(2000);
		if (!PropertyLoader.getProperties("isItApiExecution").equals("true"))
			((SeleniumWrapper) seleniumWrapperManager.get()).closeBrowser();
	}

	@AfterSuite
	public synchronized void suite() {
